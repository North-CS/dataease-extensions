    ...
    computed: {
      ...
      chart() {
        ...
        if (this.obj.chart && this.obj.chart.customAttr) {
          ...
          if (!customAttrObj.size.hasOwnProperty('innerAngle')) {
            customAttrObj.size.innerAngle = 0;
          }
          if (!customAttrObj.size.hasOwnProperty('outerAngle')) {
            customAttrObj.size.outerAngle = 0;
          }
          if (!customAttrObj.size.hasOwnProperty('depth')) {
            customAttrObj.size.depth = 60;
          }
          if (!customAttrObj.size.hasOwnProperty('columnDepth')) {
            customAttrObj.size.columnDepth = 100;
          }
          this.obj.chart.customAttr = JSON.stringify(customAttrObj);
        }
        if (this.obj.chart && this.obj.chart.customStyle) {
          let customStyle = this.obj.chart.customStyle;
          let customStyleObj = JSON.parse(customStyle);
          if (!customStyleObj.yAxis.hasOwnProperty('softMax')) {
            customStyleObj.yAxis.softMax = 100;
          }
          if (!customStyleObj.yAxis.hasOwnProperty('softMin')) {
            customStyleObj.yAxis.softMin = 0;
          }
          this.obj.chart.customStyle = JSON.stringify(customStyleObj);
        }
